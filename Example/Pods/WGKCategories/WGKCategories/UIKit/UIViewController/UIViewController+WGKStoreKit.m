...
NSString* const wgk_affiliateTokenKey = @"at";
NSString* const wgk_campaignTokenKey = @"ct";
NSString* const wgk_iTunesAppleString = @"itunes.apple.com";
...
@implementation UIViewController (WGKStoreKit)
...
- (void)wgk_presentStoreKitItemWithIdentifier:(NSInteger)itemIdentifier
{
    ...
    NSString* campaignToken = self.wgk_campaignToken ?: @"";

    NSDictionary* parameters = @{
        SKStoreProductParameterITunesItemIdentifier : @(itemIdentifier),
        wgk_affiliateTokenKey : wgk_affiliateTokenKey,
        wgk_campaignTokenKey : campaignToken,
    };

    if (self.wgk_loadingStoreKitItemBlock) {
        self.wgk_loadingStoreKitItemBlock();
    }
    [storeViewController loadProductWithParameters:parameters completionBlock:^(BOOL result, NSError* error) {
        if (self.wgk_loadedStoreKitItemBlock) {
            self.wgk_loadedStoreKitItemBlock();
        }
...
- (void)wgk_productViewControllerDidFinish:(SKStoreProductViewController*)viewController
{
...
{
    return ([URLString rangeOfString:wgk_iTunesAppleString].location != NSNotFound);
}
...
- (void)setWgk_campaignToken:(NSString*)campaignToken
{
    objc_setAssociatedObject(self, @selector(setWgk_campaignToken:), campaignToken, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSString*)wgk_campaignToken
{
    return objc_getAssociatedObject(self, @selector(setWgk_campaignToken:));
}

- (void)setWgk_loadingStoreKitItemBlock:(void (^)(void))loadingStoreKitItemBlock
{
    objc_setAssociatedObject(self, @selector(setWgk_loadingStoreKitItemBlock:), loadingStoreKitItemBlock, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void (^)(void))wgk_loadingStoreKitItemBlock
{
    return objc_getAssociatedObject(self, @selector(setWgk_loadingStoreKitItemBlock:));
}

- (void)setWgk_loadedStoreKitItemBlock:(void (^)(void))loadedStoreKitItemBlock
{
    objc_setAssociatedObject(self, @selector(setWgk_loadedStoreKitItemBlock:), loadedStoreKitItemBlock, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void (^)(void))wgk_loadedStoreKitItemBlock
{
    return objc_getAssociatedObject(self, @selector(setWgk_loadedStoreKitItemBlock:));
}
