...
@implementation UIImage (WGKFXImage)

- (UIImage *)wgk_imageCroppedToRect:(CGRect)rect
{
...
- (UIImage *)wgk_imageScaledToSize:(CGSize)size
{   
...
- (UIImage *)wgk_imageScaledToFitSize:(CGSize)size
{
    //calculate rect
    CGFloat aspect = self.size.width / self.size.height;
    if (size.width / aspect <= size.height)
    {
        return [self wgk_imageScaledToSize:CGSizeMake(size.width, size.width / aspect)];
    }
    else
    {
        return [self wgk_imageScaledToSize:CGSizeMake(size.height * aspect, size.height)];
    }
}

- (UIImage *)wgk_imageScaledToFillSize:(CGSize)size
{
    if (CGSizeEqualToSize(self.size, size))
    {
        return self;
...
- (UIImage *)wgk_imageCroppedAndScaledToSize:(CGSize)size
...
- (UIImage *)wgk_reflectedImageWithScale:(CGFloat)scale
{
	...
	//clip to gradient
	CGContextClipToMask(context, bounds, [[self class] wgk_gradientMask]);
...
- (UIImage *)wgk_imageWithReflectionWithScale:(CGFloat)scale gap:(CGFloat)gap alpha:(CGFloat)alpha
{
    //get reflected image
    UIImage *reflection = [self wgk_reflectedImageWithScale:scale];
...
- (UIImage *)wgk_imageWithShadowColor:(UIColor *)color offset:(CGSize)offset blur:(CGFloat)blur
{
    //get size
...
- (UIImage *)wgk_imageWithCornerRadius:(CGFloat)radius
{
    //create drawing context
...
- (UIImage *)wgk_imageWithAlpha:(CGFloat)alpha
{
    //create drawing context
	UIGraphicsBeginImageContextWithOptions(self.size, NO, 0.0f);
...
- (UIImage *)wgk_imageWithMask:(UIImage *)maskImage;
{
    //create drawing context
    UIGraphicsBeginImageContextWithOptions(self.size, NO, 0.0f);
    CGContextRef context = UIGraphicsGetCurrentContext();
...
- (UIImage *)wgk_maskImageFromImageAlpha
{
    //get dimensions
