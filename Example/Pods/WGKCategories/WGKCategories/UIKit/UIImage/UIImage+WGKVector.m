...
@implementation UIImage(WGKVector)
...
+ (UIImage *)wgk_iconWithFont:(UIFont *)font named:(NSString *)iconNamed withTintColor:(UIColor *)tintColor clipToBounds:(BOOL)clipToBounds forSize:(CGFloat)fontSize{
    ...
    UIImage *image = [[self wgk_cache] objectForKey:identifier];
    if(image == nil){
        ...
        if(!CGSizeEqualToSize(CGSizeZero, imageSize)){
            ...
            [[self wgk_cache] setObject:image forKey:identifier];
        }
    }
    return image;
}

+ (UIImage *)wgk_imageWithPDFNamed:(NSString *)pdfNamed forHeight:(CGFloat)height{
    return [self wgk_imageWithPDFNamed:pdfNamed withTintColor:nil forHeight:height];
}

+ (UIImage *)wgk_imageWithPDFNamed:(NSString *)pdfNamed withTintColor:(UIColor *)tintColor forHeight:(CGFloat)height{
    NSString *pdfFile = [[NSBundle mainBundle] pathForResource:pdfNamed ofType:@"pdf"];
    return [self wgk_imageWithPDFFile:pdfFile withTintColor:tintColor forSize:CGSizeMake(MAXFLOAT, height)];
}

+ (UIImage *)wgk_imageWithPDFFile:(NSString *)pdfFile withTintColor:(UIColor *)tintColor forSize:(CGSize)size{
