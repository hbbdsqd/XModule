...
@implementation UIImage (WGKBlur)
#pragma mark -
#pragma mark - Effects
...
- (UIImage *)wgk_lightImage
{
    ...
    return [self wgk_blurredImageWithSize:CGSizeMake(60, 60) tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil];
}
...
- (UIImage *)wgk_extraLightImage
{
    ...
    return [self wgk_blurredImageWithSize:CGSizeMake(40, 40) tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil];
}
...
- (UIImage *)wgk_tintedImageWithColor:(UIColor *)tintColor
{
    ...
    return [self wgk_blurredImageWithSize:CGSizeMake(20, 20) tintColor:effectColor saturationDeltaFactor:-1.0 maskImage:nil];
}
...
{
    return [self wgk_blurredImageWithSize:CGSizeMake(blurRadius, blurRadius)];
}
...
{
    return [self wgk_blurredImageWithSize:blurSize tintColor:nil saturationDeltaFactor:1.0 maskImage:nil];
}
...
//| ----------------------------------------------------------------------------
- (UIImage *)wgk_blurredImageWithSize:(CGSize)blurSize tintColor:(UIColor *)tintColor saturationDeltaFactor:(CGFloat)saturationDeltaFactor maskImage:(UIImage *)maskImage
{
...
#define ENABLE_TINT                     1
    ...
    if (hasBlur || hasSaturationChange)
    {
        ...
        if (hasBlur)
        {
            CGFloat radiusX = [self wgk_gaussianBlurRadiusWithBlurRadius:blurSize.width * inputImageScale];
            CGFloat radiusY = [self wgk_gaussianBlurRadiusWithBlurRadius:blurSize.height * inputImageScale];
        ...
        if ( (effectCGImage = vImageCreateCGImageFromBuffer(inputBuffer, &format, &wgk_cleanupBuffer, NULL, kvImageNoAllocate, NULL)) == NULL ) {
...
//
- (CGFloat)wgk_gaussianBlurRadiusWithBlurRadius:(CGFloat)blurRadius
{
...
//
void wgk_cleanupBuffer(void *userData, void *buf_data)
{ free(buf_data); }
