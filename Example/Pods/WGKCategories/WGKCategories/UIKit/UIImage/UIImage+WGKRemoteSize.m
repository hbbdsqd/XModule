...
@interface NSURL (WGKRemoteSize)
@property (nonatomic, strong) NSMutableData *wgk_sizeRequestData;
@property (nonatomic, strong) NSString *wgk_sizeRequestType;
@property (nonatomic, copy) WGKUIImageSizeRequestCompleted wgk_sizeRequestCompletion;
@end

@implementation NSURL (RemoteSize)

- (void)setWgk_sizeRequestCompletion: (WGKUIImageSizeRequestCompleted) block {
    objc_setAssociatedObject(self, &kSizeRequestCompletionKey, block, OBJC_ASSOCIATION_COPY);
}

- (WGKUIImageSizeRequestCompleted)wgk_sizeRequestCompletion {
    return objc_getAssociatedObject(self, &kSizeRequestCompletionKey);
}

- (void)setWgk_sizeRequestData:(NSMutableData *)sizeRequestData {
    objc_setAssociatedObject(self, &kSizeRequestDataKey, sizeRequestData, OBJC_ASSOCIATION_RETAIN);
}

- (NSMutableData*)wgk_sizeRequestData {
    return objc_getAssociatedObject(self, &kSizeRequestDataKey);
}

- (void)setWgk_sizeRequestType:(NSString *)sizeRequestType {
    objc_setAssociatedObject(self, &kSizeRequestTypeKey, sizeRequestType, OBJC_ASSOCIATION_RETAIN);
}

- (NSString*)wgk_sizeRequestType {
    return objc_getAssociatedObject(self, &kSizeRequestTypeKey);
}

#pragma mark - NSURLConnectionDelegate
- (void)connection:(NSURLConnection*)connection didReceiveResponse:(NSURLResponse *)response {
    [self.wgk_sizeRequestData setLength: 0];    //Redirected => reset data
}

- (void)connection:(NSURLConnection*)connection didReceiveData:(NSData *)data {
    NSMutableData* receivedData = self.wgk_sizeRequestData;
    
    if( !receivedData ) {
        receivedData = [NSMutableData data];
        self.wgk_sizeRequestData = receivedData;
    }
    ...
    if(!self.wgk_sizeRequestType ) {
        if( memcmp(pngSignature, cString, 8) == 0 ) {
            self.wgk_sizeRequestType = @"PNG";
        }
        else if( memcmp(bmpSignature, cString, 2) == 0 ) {
            self.wgk_sizeRequestType = @"BMP";
        }
        else if( memcmp(jpgSignature, cString, 2) == 0 ) {
            self.wgk_sizeRequestType = @"JPG";
        }
        else if( memcmp(gifSignature, cString, 2) == 0 ) {
            self.wgk_sizeRequestType = @"GIF";
        }
    }
    
    if( [self.wgk_sizeRequestType isEqualToString: @"PNG"] ) {
        char type[5];
        int offset = 8;
        ...
        if( strcmp(type, "IHDR") == 0 ) {   //Should always be first
            ...
            if( self.wgk_sizeRequestCompletion ) {
                self.wgk_sizeRequestCompletion(self, CGSizeMake(width, height));
            }
            
            self.wgk_sizeRequestCompletion = nil;
            
            [connection cancel];
        }
    }
    else if( [self.wgk_sizeRequestType isEqualToString: @"BMP"] ) {
        int offset = 18;
        dword width = 0, height = 0;
        memcpy(&width, cString+offset, 4);
        offset += 4;
    ...
    else if( [self.wgk_sizeRequestType isEqualToString: @"JPG"] ) {
        int offset = 4;
    ...
    else if( [self.wgk_sizeRequestType isEqualToString: @"GIF"] ) {
        int offset = 6;
        dword width = 0, height = 0;
...
-(void)connection:(NSURLConnection*)connection didFailWithError:(NSError *)error {
    if( self.wgk_sizeRequestCompletion )
        self.wgk_sizeRequestCompletion(self, CGSizeZero);
}
...
- (void)connectionDidFinishLoading: (NSURLConnection *)connection {
    ...
    if(!self.wgk_sizeRequestData.length) {
        self.wgk_sizeRequestData = nil;
    }
    else {
        //Try parse to UIImage
        UIImage* image = [UIImage imageWithData: self.wgk_sizeRequestData];
        
        if( self.wgk_sizeRequestCompletion && image) {
            self.wgk_sizeRequestCompletion(self, [image size]);
            return;
        }
    }
...
+ (void)requestSizeNoHeader:(NSURL*)imgURL completion:(WGKUIImageSizeRequestCompleted)completion{
    
    if([imgURL isFileURL] ) {
        //Load from file stream
    }
    else {
        imgURL.wgk_sizeRequestCompletion = completion;
...
+ (void)requestSizeWithHeader:(NSURL*)imgURL completion:(WGKUIImageSizeRequestCompleted)completion{
//        NSURLRequest* request = [NSURLRequest requestWithURL:imgURL];
//
