...
@implementation UIImage (WGKBetterFace)

- (UIImage *)wgk_betterFaceImageForSize:(CGSize)size
                           accuracy:(WGKAccuracy)accurary;
{
    NSArray *features = [UIImage _wgk_faceFeaturesInImage:self accuracy:accurary];
    ...
    if ([features count]==0) {
        ...
        return [self _wgk_subImageForFaceFeatures:features
                                         size:size];
    }
}

- (UIImage *)_wgk_subImageForFaceFeatures:(NSArray *)faceFeatures size:(CGSize)size
{
...
+ (NSArray *)_wgk_faceFeaturesInImage:(UIImage *)image accuracy:(WGKAccuracy)accurary
{
    CIImage *ciImage = [CIImage imageWithCGImage:image.CGImage];
    NSString *accuraryStr = (accurary == WGKAccuracyLow) ? CIDetectorAccuracyLow : CIDetectorAccuracyHigh;
