...
@implementation UIFont (WGKCustomLoader)
 ...
 */
+ (BOOL) wgk_deviceHasFullSupportForFontCollections {
 ...
 */
+ (BOOL)wgk_registerFromURL:(NSURL *)fontURL {
...
/**
 ...
 @return Registration result
 */
+ (BOOL)wgk_registerFromCGFont:(CGFontRef)fontRef {
...
+ (NSArray *)wgk_registerFontFromURL:(NSURL *)fontURL {
    // Dictionary creation
    ...
    @synchronized(appRegisteredCustomFonts) {
        ...
        if (fontPSNames == nil) {
            ...
            // Check features
            if ([UIFont wgk_deviceHasFullSupportForFontCollections]) {
                ...
                if (fontDescriptors) {
                    ...
                    if ([fontPSNames count] > 0) {
                        ...
                        if ([UIFont wgk_registerFromURL:fontURL]) {
                    ...
                    if (loadedFont != NULL) {
                        ...
                        if ([singleFontValidExtensions containsObject:[fontURL pathExtension]]) {
                            ...
                            if ([UIFont fontWithName:fontPSNames[0] size:kSizePlaceholder] == nil) {
                                ...
                                // If registration went ok
                                if ([UIFont wgk_registerFromCGFont:loadedFont]) {
                                    // Add url to this library
                                    [appRegisteredCustomFonts setObject:fontPSNames
                                                                 forKey:fontURL];
...
+ (UIFont *) wgk_customFontWithURL:(NSURL *)fontURL size:(CGFloat)size {
    ...
    NSArray *fontPSNames = [UIFont wgk_registerFontFromURL:fontURL];
...
+ (UIFont *) wgk_customFontOfSize:(CGFloat)size withName:(NSString *)name withExtension:(NSString *)extension {
    // Get url for font resource
    NSURL *fontURL = [[[NSBundle mainBundle] URLForResource:name withExtension:extension] absoluteURL];
    
    return [UIFont wgk_customFontWithURL:fontURL size:size];
}
