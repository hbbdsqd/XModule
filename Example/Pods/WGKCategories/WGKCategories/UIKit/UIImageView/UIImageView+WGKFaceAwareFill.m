...
static CIDetector * _wgk_faceDetector;


@implementation UIImageView (WGKFaceAwareFill)

+ (void)initialize
{
    _wgk_faceDetector = [CIDetector detectorOfType:CIDetectorTypeFace
                                       context:nil
...
- (void)wgk_faceAwareFill {
    // Safe check!
    if (self.image == nil) {
        return;
    }
    
    CGRect facesRect = [self wgk_rectWithFaces];
    if (facesRect.size.height + facesRect.size.width == 0)
        return;
    self.contentMode = UIViewContentModeTopLeft;
    [self wgk_scaleImageFocusingOnRect:facesRect];
}

- (CGRect) wgk_rectWithFaces {
    // Get a CIIImage
    ...
    CIDetector* detector = _wgk_faceDetector;
...
- (void) wgk_scaleImageFocusingOnRect:(CGRect) facesRect {
