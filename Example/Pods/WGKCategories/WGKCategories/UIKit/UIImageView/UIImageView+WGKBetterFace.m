...
static CIDetector *wgk_detector;

@implementation UIImageView (WGKBetterFace)

void wgk_hack_uiimageview_bf(){
    Method oriSetImgMethod = class_getInstanceMethod([UIImageView class], @selector(setImage:));
    Method newSetImgMethod = class_getInstanceMethod([UIImageView class], @selector(wgk_setBetterFaceImage:));
    method_exchangeImplementations(newSetImgMethod, oriSetImgMethod);
}

- (void)wgk_setBetterFaceImage:(UIImage *)image{
    [self setWgk_needsBetterFace:image];
    if (![self wgk_needsBetterFace]) {
        return;
    }
    
    [self wgk_faceDetect:image];
}

char nbfKey;
- (void)setWgk_needsBetterFace:(BOOL)needsBetterFace{
...
char detectorKey;
- (void)setWgk_detector:(CIDetector *)detector{
    objc_setAssociatedObject(self,
                             &detectorKey,
                             detector,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)wgk_faceDetect:(UIImage *)aImage
{
    ...
    dispatch_async(queue, ^{
        ...
        if (wgk_detector == nil) {
            NSDictionary  *opts = [NSDictionary dictionaryWithObject:[self wgk_fast] ? CIDetectorAccuracyLow : CIDetectorAccuracyHigh
                                                              forKey:CIDetectorAccuracy];
            wgk_detector = [CIDetector detectorOfType:CIDetectorTypeFace
                                                      context:nil
                                                      options:opts];
        }
        
        NSArray* features = [wgk_detector featuresInImage:image];
        
        if ([features count] == 0) {
            BFLog(@"no faces");
            dispatch_async(dispatch_get_main_queue(), ^{
                [[self wgk_imageLayer] removeFromSuperlayer];
            });
        } else {
            BFLog(@"succeed %lu faces", (unsigned long)[features count]);
            [self wgk_markAfterFaceDetect:features
...
-(void)wgk_markAfterFaceDetect:(NSArray *)features size:(CGSize)size{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        CALayer *layer = [self wgk_imageLayer];
