...
@implementation UIView (WGKAnimation)

#pragma mark - Moves

- (void)wgk_moveTo:(CGPoint)destination duration:(float)secs option:(UIViewAnimationOptions)option {
    [self wgk_moveTo:destination duration:secs option:option delegate:nil callback:nil];
}
...
- (void)wgk_raceTo:(CGPoint)destination withSnapBack:(BOOL)withSnapBack {
    [self wgk_raceTo:destination withSnapBack:withSnapBack delegate:nil callback:nil];
}
...
- (void)wgk_rotate:(int)degrees secs:(float)secs delegate:(id)delegate callback:(SEL)method {
    [UIView animateWithDuration:secs 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, wgk_radiansForDegrees(degrees));
                     }
                     completion:^(BOOL finished) { 
                         if (delegate != nil) {
#pragma clang diagnostic push
...
- (void)wgk_spinClockwise:(float)secs {
    [UIView animateWithDuration:secs/4 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, wgk_radiansForDegrees(90));
                     }
                     completion:^(BOOL finished) { 
                         [self wgk_spinClockwise:secs];
                     }];
}
...
- (void)wgk_spinCounterClockwise:(float)secs {
    [UIView animateWithDuration:secs/4 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, wgk_radiansForDegrees(270));
                     }
                     completion:^(BOOL finished) { 
                         [self wgk_spinCounterClockwise:secs];
                     }];
}
...
- (void)wgk_drainAway:(float)secs {
    self.tag = 20;
	[NSTimer scheduledTimerWithTimeInterval:secs/50 target:self selector:@selector(wgk_drainTimer:) userInfo:nil repeats:YES];
}

- (void)wgk_drainTimer:(NSTimer*)timer {
    ...
    [UIView animateWithDuration:secs/2 
                     ...
                     completion:^(BOOL finished) { 
                         [UIView animateWithDuration:secs/2 
                                          ...
                                          completion:^(BOOL finished) { 
                                              if (continuously) {
                                                  [self wgk_pulse:secs continuously:continuously];
                                              }
                                          }];
                     }];
}
