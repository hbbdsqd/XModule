...
@implementation NSString (WGKNormalRegex)
#pragma mark - 正则相关
- (BOOL)wgk_isValidateByRegex:(NSString *)regex{
...
//手机号分服务商
- (BOOL)wgk_isMobileNumberClassification{
    //运营商手机号段划分
...
//    NSString *HK = @"^[569]\\d{7}$";
    ...
    if (([self wgk_isValidateByRegex:CM])
        || ([self wgk_isValidateByRegex:CU])
        || ([self wgk_isValidateByRegex:CT])
        || ([self wgk_isValidateByRegex:PHS]))
    {
        return YES;
    }
    else
...
//中国电信: 133、153、173、177、180、181、189、199、1700(虚拟运营商号段)
    ...
    BOOL ret1 = [self wgk_isValidateByRegex:mobileRegex];
    return ret1;
}

//邮箱
- (BOOL)wgk_isEmailAddress{
    NSString *emailRegex = @"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,4}";
    return [self wgk_isValidateByRegex:emailRegex];
}

//身份证号
- (BOOL)wgk_simpleVerifyIdentityCardNum
{
    NSString *regex2 = @"^(\\d{14}|\\d{17})(\\d|[xX])$";
    return [self wgk_isValidateByRegex:regex2];
}

//车牌
- (BOOL)wgk_isCarNumber{
    //车牌号:湘K-DE829 香港车牌号码:粤Z-J499港
    NSString *carRegex = @"^[\u4e00-\u9fff]{1}[a-zA-Z]{1}[-][a-zA-Z_0-9]{4}[a-zA-Z_0-9_\u4e00-\u9fff]$";//其中\u4e00-\u9fa5表示unicode编码中汉字已编码部分，\u9fa5-\u9fff是保留部分，将来可能会添加
    return [self wgk_isValidateByRegex:carRegex];
}

- (BOOL)wgk_isMacAddress{
    NSString * macAddRegex = @"([A-Fa-f\\d]{2}:){5}[A-Fa-f\\d]{2}";
    return  [self wgk_isValidateByRegex:macAddRegex];
}

- (BOOL)wgk_isValidUrl
{
    NSString *regex = @"^((http)|(https))+:[^\\s]+\\.[^\\s]*$";
    return [self wgk_isValidateByRegex:regex];
}

- (BOOL)wgk_isValidChinese;
{
    NSString *chineseRegex = @"^[\u4e00-\u9fa5]+$";
    return [self wgk_isValidateByRegex:chineseRegex];
}

- (BOOL)wgk_isValidPostalcode {
    NSString *postalRegex = @"^[0-8]\\d{5}(?!\\d)$";
    return [self wgk_isValidateByRegex:postalRegex];
}

- (BOOL)wgk_isValidTaxNo
{
    NSString *taxNoRegex = @"[0-9]\\d{13}([0-9]|X)$";
    return [self wgk_isValidateByRegex:taxNoRegex];
}

- (BOOL)wgk_isValidWithMinLenth:(NSInteger)minLenth
...
#pragma mark - 算法相关
//精确的身份证号码有效性检测
+ (BOOL)wgk_accurateVerifyIDCardNumber:(NSString *)value {
