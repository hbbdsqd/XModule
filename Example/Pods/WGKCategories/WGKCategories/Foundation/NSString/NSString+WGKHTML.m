...
static WGK_HTMLEscapeMap wgk_gAsciiHTMLEscapeMap[] = {
...
static WGK_HTMLEscapeMap wgk_gUnicodeHTMLEscapeMap[] = {
...
static int WGK_EscapeMapCompare(const void *ucharVoid, const void *mapVoid) {
	const unichar *uchar = (const unichar*)ucharVoid;
	const WGK_HTMLEscapeMap *map = (const WGK_HTMLEscapeMap*)mapVoid;
	int val;
	if (*uchar > map->uchar) {
		val = 1;
...
@implementation NSString (WGKHTML)
#pragma --mark GTM method

- (NSString *)wgk_stringByEscapingHTMLUsingTable:(WGK_HTMLEscapeMap*)table
...
//		_GTMDevLog(@"Unable to allocate buffer or data2");
	...
	for (NSUInteger i = 0; i < length; ++i) {
		WGK_HTMLEscapeMap *val = bsearch(&buffer[i], table,
									 size / sizeof(WGK_HTMLEscapeMap),
									 sizeof(WGK_HTMLEscapeMap), WGK_EscapeMapCompare);
...
- (NSString *)wgk_stringByEscapingForHTML {
	return [self wgk_stringByEscapingHTMLUsingTable:wgk_gUnicodeHTMLEscapeMap
											 ofSize:sizeof(wgk_gUnicodeHTMLEscapeMap) 
									escapingUnicode:NO];
} // gtm_stringByEscapingHTML

- (NSString *)wgk_stringByEscapingForAsciiHTML {
	return [self wgk_stringByEscapingHTMLUsingTable:wgk_gAsciiHTMLEscapeMap
											 ofSize:sizeof(wgk_gAsciiHTMLEscapeMap) 
									escapingUnicode:YES];
} // gtm_stringByEscapingAsciiHTML

- (NSString *)wgk_stringByUnescapingFromHTML {
	...
	do {
		...
		if (length > 3 && length < 11) {
			if ([escapeString characterAtIndex:1] == '#') {
				...
				// "standard" sequences
				for (unsigned i = 0; i < sizeof(wgk_gAsciiHTMLEscapeMap) / sizeof(WGK_HTMLEscapeMap); ++i) {
					if ([escapeString isEqualToString:wgk_gAsciiHTMLEscapeMap[i].escapeSequence]) {
						[finalString replaceCharactersInRange:escapeRange withString:[NSString stringWithCharacters:&wgk_gAsciiHTMLEscapeMap[i].uchar length:1]];
						break;
					}
				}
			}
...
- (NSString *)wgk_stringWithNewLinesAsBRs {
    @autoreleasepool {
        // Strange New lines:
        //	Next Line, U+0085
        //	Form Feed, U+000C
...
- (NSString *)wgk_stringByRemovingNewLinesAndWhitespace {
    @autoreleasepool {
...
- (NSString *)wgk_stringByLinkifyingURLs {
...
- (NSString *)wgk_stringByStrippingTags {
    @autoreleasepool {
        ...
        finalString = [result wgk_stringByRemovingNewLinesAndWhitespace];
...
- (NSString *)wgk_stringByConvertingHTMLToPlainText {
    @autoreleasepool {
        // Character sets
        NSCharacterSet *stopCharacters = [NSCharacterSet characterSetWithCharactersInString:[NSString stringWithFormat:@"< \t\n\r%C%C%C%C", (unichar)0x0085, (unichar)0x000C, (unichar)0x2028, (unichar)0x2029]];
        NSCharacterSet *newLineAndWhitespaceCharacters = [NSCharacterSet characterSetWithCharactersInString:[NSString stringWithFormat:@" \t\n\r%C%C%C%C", (unichar)0x0085, (unichar)0x000C, (unichar)0x2028, (unichar)0x2029]];
        NSCharacterSet *tagNameCharacters = [NSCharacterSet characterSetWithCharactersInString:@"abcdefghijklmnopqrstuvwxyzABCDEFGHIWGKLMNOPQRSTUVWXYZ"];
        ...
        NSString *retString = [result wgk_stringByUnescapingFromHTML];
