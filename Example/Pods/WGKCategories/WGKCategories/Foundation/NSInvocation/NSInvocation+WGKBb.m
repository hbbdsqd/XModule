...
+ (id)wgk_doInstanceMethodTarget:(id)target selectorName:(NSString *)selectorName args:(NSArray *)args
{
    ...
    [invocation wgk_setArgumentsWithArray:args];
...
- (void)wgk_setArgumentsWithArray:(NSArray *)array
{
    if (nil != array && array.count > 0) {
        ...
        if (array.count != numargs) {
            ...
            [self wgk_setArgumentWithObject:arg atIndex:0];
        }else{
            for (NSUInteger i = 0; i < numargs; i++) {
                [self wgk_setArgumentWithObject:array[i] atIndex:i];
            }
        }
    }
}
