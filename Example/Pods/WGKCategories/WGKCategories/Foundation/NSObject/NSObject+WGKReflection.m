...
@implementation NSObject (WGKReflection)
...
{
    return [[self class] wgk_propertyKeys];
}
+ (NSArray *)wgk_propertyKeys {
...
{
    return [[self class] wgk_propertiesInfo];
}
/**
...
+ (NSArray *)wgk_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self wgk_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)wgk_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] wgk_propertiesInfo];
...
//            NSString *argString = [NSString stringWithCString:arg encoding:NSUTF8StringEncoding];
            [arguments addObject:[[self class] wgk_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] wgk_decodeType:returnType];
        NSString *encodeString = [[self class] wgk_decodeType:encoding];
...
-(NSDictionary *)wgk_protocolList{
    return [[self class]wgk_protocolList];
}
+ (NSDictionary *)wgk_protocolList
{
...
+ (NSArray *)wgk_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] wgk_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)wgk_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)wgk_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString wgk_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
