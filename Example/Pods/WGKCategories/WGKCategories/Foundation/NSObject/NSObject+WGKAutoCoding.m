...
static NSString *const WGKAutocodingException = @"WGKAutocodingException";
@implementation NSObject (WGKAutoCoding)
+ (BOOL)supportsSecureCoding
{
    return YES;
}
+ (instancetype)wgk_objectWithContentsOfFile:(NSString *)filePath
{
    //load the file
...
- (BOOL)wgk_writeToFile:(NSString *)filePath atomically:(BOOL)useAuxiliaryFile
{
...
+ (NSDictionary *)wgk_codableProperties
{
    //deprecated
...
- (NSDictionary *)wgk_codableProperties
{
    ...
    if (!codableProperties)
    {
        ...
        {
            [(NSMutableDictionary *)codableProperties addEntriesFromDictionary:[subclass wgk_codableProperties]];
...
- (NSDictionary *)wgk_dictionaryRepresentation
{
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    for (__unsafe_unretained NSString *key in [self wgk_codableProperties])
    {
...
- (void)wgk_setWithCoder:(NSCoder *)aDecoder
{
    BOOL secureAvailable = [aDecoder respondsToSelector:@selector(decodeObjectOfClass:forKey:)];
    BOOL secureSupported = [[self class] supportsSecureCoding];
    NSDictionary *properties = [self wgk_codableProperties];
    for (NSString *key in properties)
    {
        ...
        if (object)
        {
            ...
            {
                [NSException raise:WGKAutocodingException format:@"Expected '%@' to be a %@, but was actually a %@", key, propertyClass, [object class]];
            }
...
{
    [self wgk_setWithCoder:aDecoder];
    return self;
}
#pragma clang diagnostic pop
- (void)encodeWithCoder:(NSCoder *)aCoder
{
    for (NSString *key in [self wgk_codableProperties])
    {
