//
//  NSObject+WGKEasyCopy.m
//  NSObject-WGKEasyCopy
//
...
@implementation NSObject (WGKEasyCopy)

- (BOOL)wgk_easyShallowCopy:(NSObject *)instance
{
...
- (BOOL)wgk_easyDeepCopy:(NSObject *)instance
{
    Class currentClass = [self class];
    Class instanceClass = [instance class];
    ...
    while (instanceClass != [NSObject class]) {
        ...
        for (int i = 0; i < propertyListCount; i++) {
            ...
            if (propertyName && !readonly) {
                ...
                BOOL flag = [NSObject wgk_isNSObjectClass:propertyValueClass];
                if (flag) {
                    if ([propertyValue conformsToProtocol:@protocol(NSCopying)]) {
                        ...
                        [copyValue wgk_easyDeepCopy:propertyValue];
                        [self setValue:copyValue forKey:propertyName];
                    }
                }else{
                    [self setValue:propertyValue forKey:propertyName];
...
+ (BOOL)wgk_isNSObjectClass:(Class)clazz{
    ...
    if (flag) {
        ...
        if (!superClass) {
            ...
            return  [NSObject wgk_isNSObjectClass:superClass];
        }
    }
}
@end
