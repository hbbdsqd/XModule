...
@implementation CLLocation (WGKCH1903)


- (id) initWithCH1903x:(double)x y:(double)y
{
    self = [self initWithLatitude:[CLLocation wgk_CHtoWGSlatWithX:x y:y] longitude:[CLLocation wgk_CHtoWGSlongWithX:x y:y]];
    if (self != nil) {
...
- (double)wgk_CH1903Y {
    return [CLLocation wgk_WGStoCHyWithLatitude:self.coordinate.latitude longitude:self.coordinate.longitude];
}

- (double)wgk_CH1903X {
    return [CLLocation wgk_WGStoCHxWithLatitude:self.coordinate.latitude longitude:self.coordinate.longitude];
}


+ (double)wgk_CHtoWGSlatWithX:(double)x y:(double)y {
...
+ (double)wgk_CHtoWGSlongWithX:(double)x y:(double)y {
...
+ (double)wgk_WGStoCHyWithLatitude:(double)lat longitude:(double)lng {
    // Converts degrees dec to sex
    lat = [CLLocation wgk_decToSex:lat];
    lng = [CLLocation wgk_decToSex:lng];
    
    // Converts degrees to seconds (sex)
    lat = [CLLocation wgk_degToSec:lat];
    lng = [CLLocation wgk_degToSec:lng];
...
+ (double)wgk_WGStoCHxWithLatitude:(double)lat longitude:(double)lng {
    // Converts degrees dec to sex
    lat = [CLLocation wgk_decToSex:lat];
    lng = [CLLocation wgk_decToSex:lng];
...
+ (double)wgk_decToSex:(double)angle {
    // Extract DMS
    int deg = (int) angle;
...
+ (double)wgk_degToSec:(double)angle {
    // Extract DMS
    int deg = (int) angle;
...
+ (double)wgk_sexToDec:(double)angle {
    // Extract DMS
    int deg = (int) angle;
    int min = (int) ((angle-deg)*100);
    double sec = (((angle-deg)*100.0) - min) * 100.0;
